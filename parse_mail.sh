...
class pipermailParser:
        ...
        def __init__(self):
                ...
                self.mailURL = "http://kitty5.sh.intel.com/ubtbase"

                self.DBusername = "root"

                self.DBpassword = "intel123"

                self.DBname = "bitsmail_data"

        def start(self,project,status):

                #Fetch Content

                print "Begin Sync MailList: Status:" + str(status)
                with file("test.html") as f 
                    HomePageContent = f.read()

                HPresults = self.AnalysisHomePageContent(HomePageContent,project,status)

                MailList = self.AnalysisMailPage(project,HPresults)

                print "Finish Sync MailList: Status:" + str(status)
        ...
        def SyncDB(self,Result,Mail,project,MailLink):
                ...
                querySql = "select Subject,Sender,CreateOn from " + str(TempProject) + " where Subject = \'" + str(TempSubject) + "' and CreateOn = str_to_date('" + str(Result['CreateOn']) + "','%a %b %d %T %Y')"
                ...
                if not queryresult:
                        ...
                        sql = "insert into " + str(TempProject) + " (`Subject`,`Sender`,`CreateOn`,`Text`,`MailLink`) values \('" + str(TempSubject) + "','" + str(Mail['Sender']) + "',\str_to_date('" + str(Result['CreateOn']) + "','%a %b %d %T %Y'),'" + str(TempText) + "',\'" + str(MailLink) + "')"
